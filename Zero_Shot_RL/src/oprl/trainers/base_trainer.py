...
class BaseTrainer:
    def __init__(
        ...
        save_model_interval: int = int(5e4)
    ):
        """
        Args:
        ...
        self._logger = logger
        self.seed = seed
        self.tool_env = self._make_env_test(seed=self.seed + 188299)
        self._real_t = real_t
        self._horizen = horizen
        ...
        self.log_dir = 'projects/Neural_Motion_Simulator/Zero_Shot_RL/logs'
        self.save_model_interval = save_model_interval  

    def save_model(self, env_step: int):
        """Save model parameters to file"""
        model_path = os.path.join(self.log_dir, f'cheetah_model_step_{env_step}.pth')
        torch.save({
            'transition_model': self.transition_model.state_dict(),
            'algo': self._algo.state_dict(),
        }, model_path)
    ...
    def train(self):
        ...
        for env_step in range(self.num_steps + 1):
            ...
            next_state, reward, terminated, truncated, regul_term = self._env.step(action)
            ...
            self._logger.log_scalar("trainer/regual_reward", regul_term.item(), env_step)
            self._eval_routine(env_step, batch)
            self._visualize(env_step)
            self._save_buffer(env_step)
            self._log_stdout(env_step, batch)

            # if env_step % self.save_model_interval == 0 and env_step > 0:
            #     self.save_model(env_step)
...
def run_training(make_algo, make_env, make_env_eval, make_logger, config: dict[str, Any], seed: int):
