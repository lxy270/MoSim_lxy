...
from MoSim.src.tools import cal_prob
...
class TQC:
    ...
    def explore(self, state: npt.ArrayLike) -> npt.ArrayLike:
        state = t.tensor(state, device=self._device)
        if state.dim() == 1:  # 检查 state 是否是一维张量
    ...
    def state_dict(self):
        """Returns the state of the TQC model for saving."""
        return {
    ...
    def load_state_dict(self, state_dict):
        """Loads the saved state of the TQC model."""
        self.actor.load_state_dict(state_dict["actor"])
        self.critic.load_state_dict(state_dict["critic"])
        self.critic_target.load_state_dict(state_dict["critic_target"])
        self.log_alpha = t.tensor(state_dict["log_alpha"], requires_grad=True, device=self._device)
